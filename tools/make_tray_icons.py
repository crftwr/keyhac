...
- ``keyhac/ui/assets/MenuExtraTemplate.png`` (18 px) and ``…@2x.png`` (36 px)
  — grayscale template for the macOS menu bar extra: black ink whose alpha
  encodes the face shading, so AppKit can recolor it for menu bar state
  (dark mode, highlight). The ``…Template`` name is the AppKit convention
  puikit's ``set_tray(image=…)`` keys on.
...
def main():
    ...
    # macOS menu bar extra: 18 pt template, plus @2x for Retina.
    write_png(ASSETS / "MenuExtraTemplate.png",
              render(18, TEMPLATE_FACES, TEMPLATE_INK))
    write_png(ASSETS / "MenuExtraTemplate@2x.png",
              render(36, TEMPLATE_FACES, TEMPLATE_INK))
