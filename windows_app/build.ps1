...
# The MCP stdio bridge, as a console script.
#
# `pip install keyhac` generates a `keyhac-mcp-bridge` executable from
# pyproject's [project.scripts]. This bundle copies sources next to an embedded
# CPython instead of pip-installing, so nothing generates one - and a bundle
# user then has no command to hand an MCP client that can only spawn a
# subprocess, which is every stdio-only client including Claude Desktop.
#
# The bridge proper is keyhac-mcp-bridge.exe, compiled from src/bridge.c in
# Step 7 - an executable rather than a batch file because the MSIX install
# needs an app execution alias to be launchable at all, and an alias can only
# name an .exe (src/bridge.c explains that at length).
#
# This .cmd stays for the one reason worth keeping it: it is the path published
# in mcp.json and pasted into MCP client configs by everyone running 2.2.0 to
# 2.2.2, and deleting it would break those configs on upgrade. It forwards, so
# there is still exactly one implementation.
Info 'Writing keyhac-mcp-bridge.cmd...'
$BridgeCmd = @'
@echo off
rem keyhac-mcp-bridge - stdio -> HTTP shim for MCP clients that can only spawn a
rem subprocess. Generated by windows_app/build.ps1.
rem
rem Kept for configs written against 2.2.0-2.2.2, which named this file; the
rem bridge itself is keyhac-mcp-bridge.exe beside it. Arguments (a --config for
rem a non-default config.py) are forwarded through.
"%~dp0keyhac-mcp-bridge.exe" %*
exit /b %ERRORLEVEL%
'@ -split "`r?`n"
Set-Content -Path (Join-Path $AppRoot 'keyhac-mcp-bridge.cmd') `
            -Value $BridgeCmd -Encoding ASCII
...
# Compile + link the MCP stdio bridge -> keyhac-mcp-bridge.exe (CONSOLE
# subsystem: an MCP client talks JSON-RPC over its stdin/stdout, so unlike
# Keyhac.exe this one needs real standard handles). It embeds no Python - it
# execs runtime\python.exe - so it needs neither the Python headers nor the
# delay-load machinery; /MT keeps it free of any load-time DLL of its own.
$BridgeOut = Join-Path $AppRoot 'keyhac-mcp-bridge.exe'
$bridgeArgs = @(
    '/nologo', '/O2', '/MT', '/W3',
    (Join-Path $SrcDir 'bridge.c'),
    "/Fe:$BridgeOut",
    "/Fo:$ObjDir\",
    '/link',
    '/SUBSYSTEM:CONSOLE'
)
Info "cl.exe $($bridgeArgs -join ' ')"
& cl.exe @bridgeArgs
if ($LASTEXITCODE -ne 0) { Fail 'cl.exe failed to build the MCP bridge.' }
if (-not (Test-Path $BridgeOut)) { Fail "Bridge was not produced at $BridgeOut." }

Success "Built $BridgeOut"
