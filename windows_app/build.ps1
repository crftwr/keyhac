...
#
# Sits beside Keyhac.exe and resolves everything from %~dp0 (its own directory,
# trailing backslash included), so it works wherever the folder is installed.
# PYTHONPATH is honoured only because Step 3 deletes the embeddable's *._pth -
# with that file present the interpreter ignores PYTHONPATH entirely and the
# bridge could not find the keyhac package.
Info 'Writing keyhac-mcp-bridge.cmd...'
$BridgeCmd = @'
@echo off
rem keyhac-mcp-bridge - stdio -> HTTP shim for MCP clients that can only spawn a
rem subprocess. Generated by windows_app/build.ps1; the equivalent of the console
rem script `pip install keyhac` creates. Takes no arguments of its own; a
rem --config for a non-default config.py is forwarded through.
rem
rem python.exe, not pythonw.exe: the MCP stdio transport needs a real stdout.
setlocal
set "PYTHONPATH=%~dp0app;%~dp0Lib\site-packages"
"%~dp0runtime\python.exe" -m keyhac.mcp.bridge %*
exit /b %ERRORLEVEL%
'@ -split "`r?`n"
