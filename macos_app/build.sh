...
# Python instead of pip-installing, so nothing generates one - and a .app user
# then has no command to hand an MCP client that can only spawn a subprocess,
# which is every stdio-only client including Claude Desktop. Write it by hand.
#
# It resolves its own location rather than hardcoding /Applications: the bundle
# is routinely run from this build tree and from a mounted DMG, and a wrapper
# that only works once installed is one that fails exactly when it is being
# tested. Must land before Step 6 so the signature seals it.
log_info "Writing keyhac-mcp-bridge console script..."
BRIDGE_BIN_DIR="${RESOURCES_DIR_BUNDLE}/bin"
mkdir -p "${BRIDGE_BIN_DIR}"
cat > "${BRIDGE_BIN_DIR}/keyhac-mcp-bridge" <<'BRIDGE_EOF'
#!/bin/sh
# stdio -> HTTP shim for MCP clients that can only spawn a subprocess.
#
# Generated by macos_app/build.sh - the equivalent of the console script
# `pip install keyhac` creates. Takes no arguments of its own; a --config for a
# non-default config.py is forwarded through.
here=$(cd -- "$(dirname -- "$0")" && pwd)
resources=$(dirname -- "${here}")
contents=$(dirname -- "${resources}")
versions="${contents}/Frameworks/Python.framework/Versions"

python="${versions}/Current/bin/python3"
if [ ! -x "${python}" ]; then
    # The standard-Python embed path creates Versions/<ver> with no Current
    # symlink, so fall back to whichever single version is there.
    python=$(ls -1 "${versions}"/*/bin/python3 2>/dev/null | head -1) || true
fi
if [ ! -x "${python}" ]; then
    echo "keyhac-mcp-bridge: no embedded Python under ${versions}" >&2
    exit 1
fi

# Same two entries the launcher puts on sys.path (KeyhacAppDelegate.m): the
# package root, plus the collected third-party deps.
PYTHONPATH="${resources}:${resources}/python_packages"
export PYTHONPATH
exec "${python}" -m keyhac.mcp.bridge "$@"
BRIDGE_EOF
chmod +x "${BRIDGE_BIN_DIR}/keyhac-mcp-bridge"
log_info "  Resources/bin/keyhac-mcp-bridge"
