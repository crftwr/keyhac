...
code, not a produced glyph.  Letters, digits, space and the named editing /
cursor keys translate cleanly.  Shifted punctuation does not: which glyph
``Shift-2`` produces is a property of the keyboard layout (``@`` on ANSI,
``"`` on JIS), and Keyhac's per-layout tables map names to codes, not codes
to glyphs.
...
def to_event(key) -> Event | None:
    ...
    None means "this key has no portable PuiKit spelling" - a function key
    the route does not carry, or shifted punctuation whose glyph depends on
    the layout.  The caller drops it rather than guessing.
    """
    ...
    if len(name) == 1 and name.isdigit():
        if shift:
            # Contract §3: the shifted symbol *is* the identity - and which
            # symbol that is depends on the layout, which a vk does not say.
            return None
        return char_key_event(name, frozenset(modifiers))
