...
| `custom_condition_func(elm)` receives `UIElement` | receives `Focus` (use `focus.native` for AX) |
| `Hook`, `Console`, `Chooser`, `Clipboard` core objects | not exposed; use actions / `keymap.pop_balloon` / `getLogger` |
| `MoveWindow(x=, y=, window_edge=, screen_edge=)` | simplified: `MoveWindow(direction=, distance=)` (full edge logic returns later) |
