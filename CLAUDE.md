...
- Tray / menu-bar extra (`keyhac/ui/tray.py`): menu (console / reload / hook
  ...
  design, regenerated by `tools/make_tray_icons.py` (pure stdlib rasterizer +
  SVG emitter) into `keyhac/ui/assets/`: color `keyhac.ico` for the Windows
  tray, vector `MenuExtraTemplate.svg` for macOS (NSImage loads SVG natively
  on 11+; AppKit template, so the face shading rides in ink *opacity* — the
  original color `icon.svg` can't be used directly because template rendering
  keeps only alpha and would show a solid silhouette). Needs puikit's
  `set_tray(image=…)` (PR #82). macOS NSStatusItem state verified live;
  Windows tray icon not yet run.
